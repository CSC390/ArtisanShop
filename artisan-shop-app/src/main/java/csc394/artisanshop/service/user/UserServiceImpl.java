...
import csc394.artisanshop.repository.UserRepository;
...
public class UserServiceImpl implements UserService {
    ...
    public User add(User userCreateDto) {
        this.userRepository.save(new User(userCreateDto.getUserName(),
                ...
                userCreateDto.getUserCreateDate(),
                userCreateDto.isNotificationPermission()));
    ...
    public List<UserViewDto> getUserViewDto() {
        final List<UserViewDto> users = this.userRepository.findAll().stream().map(UserViewDto::of)
